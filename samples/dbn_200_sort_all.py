...
def chip_data_add_test(a,b):
        ...
        for x in lines:
                if int(x.find("G")) == int(-1):
                        tmp = x.split(",")
                        res = numpy.append(res,math.fabs(float(tmp[int(a)])))
	for x in lines:
        	if int(x.find("G")) == int(-1):
                        tmp = x.split(",")
                        res = numpy.append(res,math.fabs(float(tmp[int(b)])))
	...
	for y in lins:
		...
		try:
			...
			if tmp[0].strip("G").isdigit() == True & tmp[1].strip("G").isdigit() == True:
                		train = numpy.vstack((train,chip_data_add_test(int(tmp[0])-1,int(tmp[1])-1)))
...
if __name__ == "__main__":
	...
	i = 18
    	j = random.randint(1000,3500) 
	print i
	print j
    	tmp = test_dbn(0.0001,1000,1,0.0001,200,i,j)
    	f = open("run_res.txt","a+")
	f.write(str(i))
	f.write("\t")
	f.write(str(j))
	f.write("\t")
