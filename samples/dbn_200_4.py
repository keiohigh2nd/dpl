...
def gene_data():
	...
	train = numpy.array(numpy.zeros(cols*2))
        res_train = numpy.array([[0, 10,0,10]])
	...
	for y in lins:
		...
		try:
			if tmp[0].strip("G").isdigit() == True & tmp[1].strip("G").isdigit() == True:
					...
					#res_train = numpy.concatenate((res_train,numpy.array([[1,0]])))
					res_train = numpy.vstack((res_train,numpy.array([1,0,1,0])))
                		else:
                        		res_train = numpy.vstack((res_train,numpy.array([0,1,0,1])))
                	i += 1
		except:
			print "error----",y
                	i += 1
...
def gene_test():
	...
	f = open("Network3_expression_data.csv")
        ...
        res_test = numpy.array([[0, 10,0,10]])
		...
		try:
			if tmp[0].strip("G").isdigit() == True & tmp[1].strip("G").isdigit() == True:
                		...
                		if int(tmp[2].strip()) == 0:
                        		#res_test = numpy.concatenate((res_test,numpy.array([[1,0]])))
                        		res_test = numpy.vstack((res_test,numpy.array([1,0,1,0])))
                		else:
                        		res_test = numpy.vstack((res_test,numpy.array([0,1,0,1])))
                	i += 1
...
def test_dbn(pretrain_lr=0.1, pretraining_epochs=1000, k=1, \
    ...
    # construct DBN
    dbn = DBN(input=x, label=y, n_ins=num_expression, hidden_layer_sizes=[100, 400], n_outs=4, numpy_rng=rng)
